            ...
            <div className="p-6 space-y-6">
                ...
                <section>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <StockMetric label="Market Cap" value={formatMarketCap(detail.marketCap)} />
                        <StockMetric label="Dividend Yield" value={formatPercent(detail.dividendYield)} />
                ...
                <section>
                    <h2 className="text-lg font-bold text-slate-900 mb-4">Financial Highlights</h2>
                    <div className="space-y-3">
                        <div className="flex justify-between items-center py-2 border-b border-slate-100">
                            <span className="text-sm text-slate-600">Revenue (TTM)</span>
                            <span className="text-sm font-semibold text-slate-900">{formatRevenue(detail.revenue)}</span>
                        </div>
                        <div className="flex justify-between items-center py-2 border-b border-slate-100">
                            <span className="text-sm text-slate-600">Profit Margin</span>
                            <span className="text-sm font-semibold text-slate-900">{formatPercent(detail.profitMargin)}</span>
                        </div>
                        <div className="flex justify-between items-center py-2 border-b border-slate-100">
                            <span className="text-sm text-slate-600">Operating Margin</span>
                            <span className="text-sm font-semibold text-slate-900">{formatPercent(detail.operatingMargin)}</span>
                        </div>
                        <div className="flex justify-between items-center py-2">
                            <span className="text-sm text-slate-600">Return on Equity</span>
                            <span className="text-sm font-semibold text-slate-900">{formatPercent(detail.returnOnEquity)}</span>
                        </div>
                    </div>
                </section>
                ...
                <section className="pt-4 border-t border-slate-200">
                    <div className="flex gap-6 text-xs text-slate-500">
                        <span>Country: <span className="font-medium text-slate-700">{detail.country}</span></span>
                        <span>Currency: <span className="font-medium text-slate-700">{detail.currency}</span></span>
                    </div>
                </section>
            </div>
        </div>
